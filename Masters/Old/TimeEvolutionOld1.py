...
def MomDataCalc( N):
    ...
    H = np.linspace(0, 1, N)
      ...
      # rd,td
    '''TempMomData[0:2, :] = V1 + H * Vrel'''
    # Finding The Collision Angles, skipping parents
    for i in range(0, N):
        TempMomData[0:2, i] = V1 + H[i] * Vrel
        TempMomData[2, i] = nplg.norm(TempMomData[0:2, i])  # v
        TempMomData[3, i] =  nplg.norm(TempMomData[0:2, i] - V1)  # H[i]*vrel#vrel1
        TempMomData[4, i] = nplg.norm( V2-TempMomData[0:2, i] ) # (1-H[i])*vrel  #vrel2
    ...
    MomentaCol = np.zeros((2, N))  # orbit dependant part, 1 for each parent
    ...
    RcolParents[0, :] =  MomentaCol[0, :]*PConstant * ParentConstant[0]  * (dfSize ** 2.)
    RcolParents[1, :] =  MomentaCol[1, :]*PConstant * ParentConstant[1]  * (dfSize ** 2.)
