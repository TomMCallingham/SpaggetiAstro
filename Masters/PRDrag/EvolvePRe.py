...
from Masters.ComponentSpaceAnalysis.EccentricitySolver import *
...
def GraphPrEvolve(a,e,T,Tini):
    ...
    for i in range(-3,3):
        ...
        #plt.loglog(Data[0, :] / year, Data[1, :] / au, label=r"$R_{\textnormal{Frag}}$"+' =10^%s' % i)  # a plot
    ...
    plt.title('Shrinking Eccentricity')
    plt.xlim([1e4, 1e8])
    plt.savefig('Shrinke.png')
    #plt.title('a=%s au, e=%s'%(a/au,e))
    plt.figure(1)
    #plt.title(' Radiation shrinking a Tini=%.E'%Tini)
    plt.ylabel('a/au')
    plt.title('Shrinking Semi-major Axis')
    plt.xlabel('Time/years')
    #plt.title('a=%s au, e=%s' % (a/au, e))
    plt.ylim([(RocheRad/au)*(1/10),(2*a)/au])
    plt.xlim([1e4,1e8])
    plt.legend()
    plt.savefig('Shrinka.png')
    #plt.show()
    return
