...
def MomentaData(x,a1,e1,s1,a2,e2,s2):
    ...
    H = np.linspace(0, 1, N)
    MomentasData[0:2,:] = V1 + H * Vrel #rd,td


    #Finding The Collision Angles, skipping parents
    for i in range(0,N):
        MomentasData[2, i] = nplg.norm(MomentasData[0:2, i])  # v
        MomentasData[3, i] = nplg.norm(MomentasData[0:2, i] - V1)  # vrel1
        MomentasData[4, i] = nplg.norm(MomentasData[0:2, i] - V2)  # vrel2
    ...
    #Anlgeplot
    plt.figure()
    plt.plot(H[1:N-1],MomentasData[5, 1:N - 1],label='Angle1')
    plt.plot(H[1:N - 1], MomentasData[6, 1:N - 1], label='Angle2')
    plt.legend()
...
MomentaData('a',2*au,0.99,0,2.1*au,0.993,1.5)
