...
def intersectfinder(): #note initiial has sli=1
    ...
    dRplus=npr(a1,e1,(pi/2)-s1i-(wp1*(10**-6.)*T))-npr(a2, e2, (pi / 2) - s2i - (wp2*(10**-6.) * T))   #changed from plue to minus!
    dRminus = npr(a1, e1, -(pi / 2) - s1i - (wp1*(10**-6.)*T))- npr(a2, e2, -(pi / 2)- s2i- (wp2*(10**-6.) * T))
    ...
    for t in range(0,int((Tmax/tinterval)-1)):
        if dRplus[t]*dRplus[t+1]<0: # if sign change
            s1=(s1i+(wp1*(10**-6.)*t))%(2*pi) #+pi/2?
            s2= (s2i + (wp2*(10**-6.) * t))%(2*pi)
...
def lambdapattern():
    ...
    plt.ylabel('lambda/pi in rad')
    plt.xlim([0, 20e6])
    plt.title('Value of Lambda at the Collision Points, ab colour')
...
#Pp=(2*pi)/(wp1-wp2)
#print('PatternPeriod',Pp),print('no in 100',100/Pp),print('Pattern Density',4*(Pp/min(Tp1,Tp2))), print('expect in a 100:',400/min(Tp1,Tp2))
