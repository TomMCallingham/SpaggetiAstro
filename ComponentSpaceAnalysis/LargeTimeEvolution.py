...
def MomDataCalc(x,a1,e1,s1,a2,e2,s2,N):
   ...
   #CollisionPoints
    ...
    MomentaCol[0,1:N]=(TempMomData[3,1:N]/(TempMomData[2,1:N]*((TempMomData[1,1:N]/R)**3.)*abs(np.sin(TempMomData[5,1:N])))) #parent1
    MomentaCol[1, 0:N-1] = (TempMomData[4, 0:N-1] / (TempMomData[2, 0:N-1] * ((TempMomData[1, 0:N-1] / R) ** 3.) * abs(np.sin(TempMomData[6, 0:N-1]))))#paernt2
...
def Evolution(x,a1,e1,s1,a2,e2,s2):
    ...
    for t in range(0,T):
        #print('t=',t+1)
        Dist[:,:,t+1]=Dist[:,:,t]  #new=old with changes

        # Parent

        for fp in range(1, J + 1):  # N-1 parent
            for f in range(1, J + 1):
                # find output bin
                ncol = int(round(MassFraction[fp, f] * (N - 1)))
                # ColRate
                Rcol = MomData[5 , N-1] * CrossSec[f, fp] * Dist[f, 0, t] * Dist[fp, N-1, t]
                #print (Rcol)
                #changes
                Dist[f, ncol, t + 1] +=  Rcol
                Dist[fp, ncol, t + 1] +=  Rcol
                Dist[ f, 0, t + 1] -= Rcol  # can get minus number of fragments, needs to remain super large
                Dist[fp, N-1, t + 1] -= Rcol


        #child loop
        for p in (0,1):  #parents 1 or 2
            for n in range(1,N-1):
                for fp in range(1,J+1):#note 0th bin is total
                    for f in range(1,J+1):
                        # find output bin
                        ncol = int(n +round( MassFraction[fp, f] * (nparent[p] - n)))
                        #ColRate
                        Rcol=MomData[5+p,n]*CrossSec[f,fp]*Dist[f,n,t]*Dist[fp,nparent[p],t]

                        #changes
                        Dist[f,ncol,t+1]+=Rcol
                        Dist[fp, ncol, t + 1] +=  Rcol
                        
                        Dist[f, n, t + 1] -=   Rcol #can get minus number of fragments, needs to remain super large
                        Dist[fp, nparent[p], t + 1] -=   Rcol
        ...
        #Total Mass Sum
        for f in range(1, J+1):
            Dist[0,:,t+1]=Dist[0,:,t+1]+ Dist[f,:,t+1]*(f**3.)

    '''H = np.linspace(0, 1, N)

    plt.figure()
    plt.plot(H, (Dist[0, :, 0]), label='Ininal Distribution log')
    plt.legend()




    for t in np.linspace(200,T,4):
        plt.figure()
        plt. plot(H,(Dist[0,:,i]), label= 't=%s'%t)
        plt.legend()
    #plt.plot(H, (Dist[0, :, T]), label='final')
    #plt.legend()
    plt.show()'''
