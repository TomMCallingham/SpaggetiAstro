...
def MinimalEccentricitySGraph(a1,e1,a2,e2,Mstar,m1,m2,N,Ns):
    ...
    for s2 in range(1,Ns):
        #print('s2=',s2*steps)
        MinGen = MinimalCircularisation(a1, e1, s1, a2, e2, np.around((steps*s2+0.01), decimals=2), Mstar, m1, m2, N)
        plt.plot(Generation, MinGen[:, 5],label='Lambda=%s'%(np.around((steps*s2+0.01), decimals=2)))
